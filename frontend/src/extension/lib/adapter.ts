...
export function createEndpoint(
  ...
  async function deserialize(data: any, ports: any[]): Promise<any> {
    ...
    // console.log("am i sending back data here?", data);
    return data;
  }
  ...
  return {
    ...
    addEventListener: (_, handler) => {
      const listener = async (data: any) => {
        ...
        const thedata = await deserialize(data, ports);
        // This is a nasty hack to get the tabId into the message
        if (
          port.sender?.tab?.id &&
          thedata.type === "APPLY" &&
          typeof thedata.argumentList[0] === "object" &&
          thedata.argumentList[0]?.type === "RAW" &&
          thedata.argumentList[0]?.value &&
          typeof thedata.argumentList[0]?.value === "object" &&
          typeof thedata.argumentList[0]?.value?.tabId === "undefined"
        ) {
          thedata.argumentList[0].value.tabId = port.sender?.tab?.id;
        }
