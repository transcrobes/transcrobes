...
export class AccessHandlePoolVFS extends VFS.Base {
  ...
  async #acquireAccessHandles() {
    ...
    // @ts-ignore this looks like a typing bug - it looks like https://developer.mozilla.org/en-US/docs/Web/API/FileSystemDirectoryHandle
    // has a Symbol.iterator method that returns an AsyncIterator
   ...
   */
  #getAssociatedPath(accessHandle): string {
  ...
  #setAssociatedPath(accessHandle: FileSystemSyncAccessHandle, path: string, flags: number) {
    ...
    if ((encodedResult.written || 0) >= HEADER_MAX_PATH_SIZE) {
   ...
   */
  #computeDigest(corpus: Uint8Array): ArrayBuffer {
    if (!corpus[0]) {
