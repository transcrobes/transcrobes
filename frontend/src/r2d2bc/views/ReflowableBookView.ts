...
export default class ReflowableBookView implements BookView {
  ...
  stop(): void {
    ...
    let doc = this.iframe.contentDocument;
    if (doc) {
      const body = HTMLUtilities.findIframeElement(this.iframe.contentDocument, "body") as HTMLBodyElement;
  ...
  atEnd(): boolean {
    if (this.scrollMode) {
      const wrapper = HTMLUtilities.findRequiredElement(document, "#iframe-wrapper");
      return Math.ceil(this.scrollingElement.scrollHeight - wrapper.scrollTop) - 1 <= BrowserUtilities.getHeight();
    } else {
  ...
  get scrollWidth() {
    ...
    // For some reason, they used this, but I don't understand why...
    // return pages * width;
    return scrollWidth;
  }
}
