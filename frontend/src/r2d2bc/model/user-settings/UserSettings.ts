...
export class UserSettings implements IUserSettings {
  async isPaginated() {
    let scroll = await this.getPropertyAndFallback<Switchable>("verticalScroll", ReadiumCSS.SCROLL_KEY);
  ...
  public static async create(config: UserSettingsConfig): Promise<any> {
    ...
    if (config.initialUserSettings) {
      ...
      if (initialUserSettings.textAlignment) {
        ...
        let prop = settings.userProperties.getByRef(ReadiumCSS.TEXT_ALIGNMENT_REF);
        if (prop) {
      ...
      if (initialUserSettings.columnCount) {
        ...
        let prop = settings.userProperties.getByRef(ReadiumCSS.COLUMN_COUNT_REF);
        if (prop) {
      ...
      if (initialUserSettings.wordSpacing) {
        ...
        let prop = settings.userProperties.getByRef(ReadiumCSS.WORD_SPACING_REF);
        if (prop) {
      ...
      if (initialUserSettings.letterSpacing) {
        ...
        let prop = settings.userProperties.getByRef(ReadiumCSS.LETTER_SPACING_REF);
        if (prop) {
      ...
      if (initialUserSettings.pageMargins) {
        ...
        let prop = settings.userProperties.getByRef(ReadiumCSS.PAGE_MARGINS_REF);
        if (prop) {
  ...
  private async initialise() {
    this.appearance = await this.getPropertyAndFallback<Enumerable>("appearance", ReadiumCSS.APPEARANCE_KEY);

    this.verticalScroll = await this.getPropertyAndFallback<Switchable>("verticalScroll", ReadiumCSS.SCROLL_KEY);
    this.fontFamily = await this.getPropertyAndFallback<Enumerable>("fontFamily", ReadiumCSS.FONT_FAMILY_KEY);
    ...
    //       )) as Switchable).value
    //     : this.publisherDefaults;
    this.textAlignment = await this.getPropertyAndFallback<Enumerable>("textAlignment", ReadiumCSS.TEXT_ALIGNMENT_KEY);
    this.columnCount = await this.getPropertyAndFallback<Enumerable>("columnCount", ReadiumCSS.COLUMN_COUNT_KEY);

    this.fontSize = await this.getPropertyAndFallback<Incremental>("fontSize", ReadiumCSS.FONT_SIZE_KEY);

    this.wordSpacing = await this.getPropertyAndFallback<Incremental>("wordSpacing", ReadiumCSS.WORD_SPACING_KEY);
    this.letterSpacing = await this.getPropertyAndFallback<Incremental>("letterSpacing", ReadiumCSS.LETTER_SPACING_KEY);
    this.pageMargins = await this.getPropertyAndFallback<Incremental>("pageMargins", ReadiumCSS.PAGE_MARGINS_KEY);
    this.lineHeight = await this.getPropertyAndFallback<Incremental>("lineHeight", ReadiumCSS.LINE_HEIGHT_KEY);
  ...
  private async reset() {
    ...
    if (doc) {
      const html = HTMLUtilities.findIframeElement(doc, "html") as HTMLHtmlElement;
      if (html) {
  ...
  private async initializeSelections(): Promise<void> {
    if (this.headerMenu) this.settingsView = HTMLUtilities.findElement(this.headerMenu, "#container-view-settings");
  }
      ...
      if (html) {
        ...
        if (this.view?.delegate.publication.isReflowable) {
          ...
          if (await this.getProperty(ReadiumCSS.FONT_SIZE_KEY)) {
            html.style.setProperty(
              ...
              this.userProperties.getByRef(ReadiumCSS.FONT_SIZE_REF)?.toString() ?? null,
            );
          }
          // Apply word spacing
          if (await this.getProperty(ReadiumCSS.WORD_SPACING_KEY)) {
            ...
            html.style.setProperty(
              ...
              this.userProperties.getByRef(ReadiumCSS.WORD_SPACING_REF)?.toString() ?? null,
            );
          }
          // Apply letter spacing
          if (await this.getProperty(ReadiumCSS.LETTER_SPACING_KEY)) {
            ...
            html.style.setProperty(
              ...
              this.userProperties.getByRef(ReadiumCSS.LETTER_SPACING_REF)?.toString() ?? null,
            );
          }
        }
        // Apply column count
        ...
        if (this.view?.delegate.publication.isReflowable) {
          ...
          if (await this.getProperty(ReadiumCSS.TEXT_ALIGNMENT_KEY)) {
            if (this.userProperties.getByRef(ReadiumCSS.TEXT_ALIGNMENT_REF)?.toString() === "auto") {
              ...
              html.style.setProperty(
                ...
                this.userProperties.getByRef(ReadiumCSS.TEXT_ALIGNMENT_REF)?.toString() ?? null,
              );
            }
          }
          ...
          if (await this.getProperty(ReadiumCSS.LINE_HEIGHT_KEY)) {
            ...
            html.style.setProperty(
              ...
              this.userProperties.getByRef(ReadiumCSS.LINE_HEIGHT_REF)?.toString() ?? null,
            );
          }
          // Apply page margins
          if (await this.getProperty(ReadiumCSS.PAGE_MARGINS_KEY)) {
            ...
            html.style.setProperty(
              ...
              this.userProperties.getByRef(ReadiumCSS.PAGE_MARGINS_REF)?.toString() ?? null,
            );
          }
        }
        ...
        if (await this.getProperty(ReadiumCSS.APPEARANCE_KEY)) {
          html.style.setProperty(
            ...
            this.userProperties.getByRef(ReadiumCSS.APPEARANCE_REF)?.toString() ?? null,
          );
        ...
        if (this.view?.delegate.publication.isReflowable) {
          ...
          if (await this.getProperty(ReadiumCSS.FONT_FAMILY_KEY)) {
            html.style.setProperty(
              ...
              this.userProperties.getByRef(ReadiumCSS.FONT_FAMILY_REF)?.toString() ?? null,
            );
            if (this.userProperties.getByRef(ReadiumCSS.FONT_FAMILY_REF)?.value === 0) {
              ...
              if (prop) {
                HTMLUtilities.setAttr(html, "data-viewer-font", prop.toString());
              }
              html.style.setProperty(ReadiumCSS.FONT_OVERRIDE_KEY, "readium-font-on");
            }
          } else {
  ...
  private async updateUserSettings() {
    let userSettings = {
      ...
      textAlignment:
        UserSettings.textAlignmentValues[await this.userProperties?.getByRef(ReadiumCSS.TEXT_ALIGNMENT_REF)?.value],
      columnCount:
        UserSettings.columnCountValues[await this.userProperties?.getByRef(ReadiumCSS.COLUMN_COUNT_REF)?.value],
  ...
  async getProperty<T extends UserProperty = UserProperty>(name: string): Promise<T | null> {
  ...
  get currentSettings() {
    return {
      ...
      textAlignment:
        UserSettings.textAlignmentValues[this.userProperties?.getByRef(ReadiumCSS.TEXT_ALIGNMENT_REF)?.value], //"auto", "justify", "start"
  ...
  async increase(incremental: UserSettingsIncrementable): Promise<void> {
    if (incremental === "fontSize") {
      ...
      this.fontSize = this.userProperties?.getByRef(ReadiumCSS.FONT_SIZE_REF)?.value;
      let prop = this.userProperties?.getByRef(ReadiumCSS.FONT_SIZE_REF);
      if (prop) {
        await this.storeProperty(prop);
      }
      ...
      this.letterSpacing = this.userProperties?.getByRef(ReadiumCSS.LETTER_SPACING_REF)?.value;
      let prop = this.userProperties?.getByRef(ReadiumCSS.LETTER_SPACING_REF);
      if (prop) {
        await this.storeProperty(prop);
      }
      ...
      this.wordSpacing = this.userProperties?.getByRef(ReadiumCSS.WORD_SPACING_REF)?.value;
      let prop = this.userProperties?.getByRef(ReadiumCSS.WORD_SPACING_REF);
      if (prop) {
        await this.storeProperty(prop);
      }
      ...
      this.lineHeight = this.userProperties?.getByRef(ReadiumCSS.LINE_HEIGHT_REF)?.value;
      let prop = this.userProperties?.getByRef(ReadiumCSS.LINE_HEIGHT_REF);
      if (prop) {
        await this.storeProperty(prop);
      }
  ...
  async decrease(incremental): Promise<void> {
    if (incremental === "fontSize") {
      ...
      this.wordSpacing = this.userProperties?.getByRef(ReadiumCSS.LINE_HEIGHT_REF)?.value;
      let prop = this.userProperties?.getByRef(ReadiumCSS.LINE_HEIGHT_REF);
      if (prop) {
        await this.storeProperty(prop);
      }
