...
export class TTSModule2 implements ReaderModule {
    ...
    if (window.speechSynthesis.speaking && this.speaking && startX === this.startX && startY === this.startY) {
      ...
      if (doc) {
        ...
        if (selection.isCollapsed) {
          let doc = this.delegate.iframes[0].contentDocument;
          const selectionInfo = this.delegate.annotationModule?.annotator?.getTemporarySelectionInfo(doc);
        ...
        // Find ending point
        do {
          range.setEnd(node, node.length < range.endOffset ? range.endOffset + 1 : node.length);
        } while (range.toString().indexOf(" ") === -1 && range.toString().trim() !== "");

        let iframe = document.querySelector("main#iframe-wrapper iframe") as HTMLIFrameElement;
    ...
    if (partial) {
      ...
      if (doc) {
        ...
        if (selection.isCollapsed) {
          ...
          const selectionInfo = self.delegate.annotationModule?.annotator?.getTemporarySelectionInfo(doc);
          selection.addRange(selectionInfo.range);
        }
        ...
        if (rootEl) {
          ...
          if (ttsQueueItem && selectionInfo && selectionInfo.cleanText) {
            ...
            log.log(textToBeSpoken, selectionInfo.range?.commonAncestorContainer.textContent);
      ...
      if (initialVoice === undefined) {
        initialVoice =
          this.tts.voice && this.tts.voice.lang
            ? this.voices.filter((v: any) => v.lang.replace("_", "-") === this.tts.voice.lang)[0]
            : undefined;
      }
    } else {
      initialVoice =
    ...
    if (publicationVoiceHasHyphen === true) {
      publicationVoice =
        this.tts.voice && this.tts.voice.usePublication
          ? this.voices.filter((v: any) => {
              var lang = v.lang.replace("_", "-");
              return (
                lang.startsWith(self.delegate.publication.Metadata.Language[0]) ||
      ...
      publicationVoice =
        this.tts.voice && this.tts.voice.usePublication
          ? this.voices.filter((v: any) => {
              return (
                v.lang.startsWith(self.delegate.publication.Metadata.Language[0]) ||
        ...
        if (idxEnd > idx) {
          ...
          if (idx !== idxEnd) {
            ...
            if (ttsQueueItem) {
              ...
              utterance.onboundary = (ev: SpeechSynthesisEvent) => {
                self.updateTTSInfo(ttsQueueItem, ev.charIndex, ev.charLength, 0, utterance.text);
              };
              setTimeout(() => {
                window.speechSynthesis.speak(utterance);
              }, 0);
  ...
  speakPlay() {
    ...
    if (rootEl) {
      ...
      function findVisibleText() {
        ...
        let doc = self.delegate.iframes[0].contentDocument;
        if (doc) {
          const range = self.highlighter.dom(doc.body).getWindow().document.createRange();

          const selection = self.highlighter.dom(self.delegate.iframes[0].contentDocument?.body).getSelection();
          selection.removeAllRanges();
  ...
  protected async start(): Promise<void> {
    ...
    setTimeout(() => {
      this.properties?.hideLayer ? this.delegate.hideLayer("readaloud") : this.delegate.showLayer("readaloud");
    }, 10);
  }
  ...
  wrapHighlightWord(
    ...
    if (rangeStartNode && rangeEndNode) {
      ...
      if (result) {
        ...
        const viewportOffset = (result[1]?.firstChild as HTMLElement)?.getBoundingClientRect();
