...
import { TextHighlighter, _highlights, CLASS_HIGHLIGHT_AREA, HighlightContainer } from "./highlight/TextHighlighter";
import { ReaderModule } from "./ReaderModule";
import { addEventListenerOptional } from "../utils/EventHandler";
import { HighlightType, IHighlight } from "./highlight/common/highlight";
import { Annotation, AnnotationMarker, Bookmark, Locator } from "../model/Locator";
...
import { _getCssSelectorOptions, ISelectionInfo } from "./highlight/common/selection";
...
export class AnnotationModule implements ReaderModule {
  ...
  protected async start(): Promise<void> {
    ...
    setTimeout(() => {
      this.properties?.hideLayer ? this.delegate.hideLayer("highlights") : this.delegate.showLayer("highlights");
    }, 10);
  }
  private hide: HTMLLinkElement = HTMLUtilities.findElement(document, "#menu-button-hide");
  private show: HTMLLinkElement = HTMLUtilities.findElement(document, "#menu-button-show");

  hideAnnotationLayer() {
    let doc = this.delegate.iframes[0].contentDocument;
    if (doc) {
      const container = HTMLUtilities.findElement(doc, "#R2_ID_HIGHLIGHTS_CONTAINER");
      if (container) {
    ...
    return new Promise(async (resolve) => {
      ...
      if (this.rights.enableAnnotations) {
        setTimeout(() => {
          this.drawHighlights();
          this.showHighlights();
          addEventListenerOptional(this.delegate.iframes[0].contentDocument?.body, "click", this.click.bind(this));
        }, 300);
      }
      resolve(null);
    });
  ...
  async scrollToHighlight(id: any): Promise<any> {
    ...
    var element = await this.annotator?.getAnnotationElement(id, this.delegate.iframes[0].contentWindow as any);
        ...
        if (tocItem) {
          ...
          if ((this.rights.autoGeneratePositions && this.publication.positions) || this.publication.positions) {
            const positions = this.publication.positionsByHref(
              this.publication.getRelativeHref(this.delegate.currentChapterLink.href),
            );
            const positionIndex = Math.ceil((progression ?? 0) * (positions.length - 1));
    ...
    if (this.annotator) {
      ...
      if (highlights) {
        highlights = highlights.filter(
          (rangeRepresentation) => rangeRepresentation.highlight.marker !== AnnotationMarker.Bookmark,
        );
      ...
      if (this.api) {
        ...
        if (this.highlighter && highlights && this.delegate.iframes[0].contentDocument?.readyState === "complete") {
          ...
          for (const rangeRepresentation of highlights) {
            ...
            if (this.delegate.currentTocUrl !== undefined) {
              tocItem = this.publication.getTOCItem(this.delegate.currentTocUrl);
            }
            ...
            if (tocItem) {
              ...
              if (annotation.href === href) {
                ...
                setTimeout(async () => {
                  if (annotation.highlight?.marker === AnnotationMarker.Underline) {
                    ...
                    addEventListenerOptional(comment, "click", (event: MouseEvent) => {
              ...
              if (annotation.href === href) {
                ...
                setTimeout(async () => {
                  if (annotation.highlight?.marker === AnnotationMarker.Underline) {
                    ...
                    let comment: HTMLDivElement = document.createElement("div");
                    comment.innerHTML = commentTemplate;
                    (comment.childNodes[0] as HTMLDivElement).innerHTML = IconLib.note;
                    addEventListenerOptional(comment, "click", (event: MouseEvent) => {
                      event.preventDefault();
                      event.stopPropagation();
                      this.scrollToHighlight(annotation.id);
  ...
  repositionGutters(): any {
    let doc = this.delegate.iframes[0].contentDocument;
    if (doc) {
      this.commentGutter = doc.getElementById(HighlightContainer.R2_ID_GUTTER_RIGHT_CONTAINER) as HTMLDivElement;
        ...
        if (this.annotator) {
          ...
          if (highlights) {
            ...
            highlights.forEach((rangeRepresentation) => {
              ...
              addEventListenerOptional(highlightAreaIcon, "click", (event: MouseEvent) => {
                event.preventDefault();
                event.stopPropagation();
            ...
            annotations.forEach(function (locator: any) {
              ...
              if (link.Href && locator.href.endsWith(href)) {
                ...
                if (type === AnnotationType.Annotation) {
                  ...
                  if ((locator as Annotation).highlight?.marker === AnnotationMarker.Underline) {
                    if (typeof (locator as Annotation).highlight?.color === "object") {
                  ...
                  let formattedProgression =
                    Math.round((locator.locations.progression ?? 0) * 100) + "% " + "through resource";
  ...
  reposition(highlights: Array<any>) {
    ...
    const positionAnnotations = (newArray: Array<any>, currentElement: any, currentIndex: number) => {
...
export function repositionHighlights(highlights: Array<any>) {
  const positionAnnotations = (newArray: Array<any>, currentElement: any, currentIndex: number) => {
    ...
    const preY = newArray[currentIndex - 1] && newArray[currentIndex - 1].position;

    const preHeight = newArray[currentIndex - 1] && 24;

    const preBottomY = currentIndex === 0 ? 0 : preY + preHeight + 0;
    const newY = preBottomY > originY && currentElement?.icon?.position === "right" ? preBottomY : originY;
