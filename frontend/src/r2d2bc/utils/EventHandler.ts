...
export default class EventHandler {
  ...
  public setupEvents(element: HTMLElement | Document | null) {
    if (element !== null) {
      ...
      element.addEventListener("click", this.handleLinks.bind(this) as any);
    } else {
  ...
  private linkInPublication = (readingOrder: Link[], clickedHref: string) =>
    readingOrder.some((link: Link) => {
      return (
        !link.Rel?.includes("external") && this.navigator.publication.getRelativeHref(clickedHref).includes(link.Href)
      );
    });
